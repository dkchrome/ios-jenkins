//
//  UIStringConstants.swift
//  StockFeed
//
//  Created by Deepak Kumar on 14/04/21.
//

import Foundation

//General
let noInternetMsg = NSLocalizedString("You are not connected to internet. Please try again.", comment: "")
let errorTitle = NSLocalizedString("Error", comment: "")
let enterDetailsMsg = NSLocalizedString("Please enter all details.", comment: "")
let nameEmptyMsg = NSLocalizedString("Please enter User name.", comment: "")
let emailInvalidMsg = NSLocalizedString("Please enter email in correct format.", comment: "")
let passwordInvalidMsg = NSLocalizedString("Please enter password correctly.", comment: "")

//Texts
let nameTxt = NSLocalizedString("Name", comment: "")
let emailTxt = NSLocalizedString("Email", comment: "")
let passwordTxt = NSLocalizedString("Password", comment: "")
let signupTxt = NSLocalizedString("Sign Up", comment: "")
let okTxt = NSLocalizedString("Ok", comment: "")
let stocksTxt = NSLocalizedString("Stocks", comment: "")

//Placeholders
let enterNameTxt = NSLocalizedString("Enter Name", comment: "")
let enterEmailTxt = NSLocalizedString("Enter Email", comment: "")
let enterPasswordTxt = NSLocalizedString("Enter Password", comment: "")
